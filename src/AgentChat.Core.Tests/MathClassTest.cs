...
",
            ...
            {
                { CreateMathQuestionFunction, CreateMathQuestionWrapper },
                { AnswerIsCorrectFunction, AnswerIsCorrectWrapper }
            });
...
",
            functionMap: new Dictionary<FunctionDefinition, Func<string, Task<string>>>
            {
                { AnswerQuestionFunction, AnswerQuestionWrapper }
            });
...
",
            ...
            .CreateAutoReplyAgent("Admin", async (msgs, ct) =>
            {
                ...
                {
                    return new Message(Role.Assistant, IChatMessageExtension.TERMINATE, "Admin");
                }
...
-end-
", group);
        var chatHistory = await admin.SendMessageToGroupAsync(group, "the number of resolved question is 0", 30);
