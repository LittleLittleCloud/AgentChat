...
public static class AgentExtension
{
    ...
    /// <param name="autoReplyMessageFunc">
    ///     function to determine the auto reply message.
    ///     If the function returns a message, that message will be sent as auto reply.
    ///     If the function returns null, the <paramref name="agent" /> will be called to generate the reply message.
    /// </param>
    /// <returns>
    ///     <see cref="AutoReplyAgent" />
    /// </returns>
    ...
    public static PostProcessAgent CreatePostProcessAgent(
        ...
        new(innerAgent, name, postprocessFunc);
    ...
    public static PreprocessAgent CreatePreprocessAgent(
        ...
        new(innerAgent, name, preprocessFunc);

    public static async Task<IChatMessage> SendMessageAsync(
        this IAgent agent,
        CancellationToken ct = default) =>
        await agent.SendMessageAsync(Enumerable.Empty<IChatMessage>(), ct);
    ...
    public static async Task<IChatMessage> SendMessageAsync(
        ...
        CancellationToken ct = default) =>
        await agent.CallAsync(history, ct);
    ...
    public static async Task<IEnumerable<IChatMessage>> SendMessageToAgentAsync(
        ...
        CancellationToken ct = default) =>
        await agent.SendMessageToAgentAsync(receiver, Enumerable.Empty<IChatMessage>(), maxRound, throwWhenMaxRoundReached, ct);
    ...
    public static async Task<IEnumerable<IChatMessage>> SendMessageToAgentAsync(
        ...
        CancellationToken ct = default)
    {
        var chatMessage = new Message(Role.User, message, agent.Name);
    ...
    public static async Task<IEnumerable<IChatMessage>> SendMessageToGroupAsync(
        ...
        CancellationToken ct = default)
    {
        var chatMessage = new Message(Role.User, msg, agent.Name);
